...
void selection_sort(int* A, int n); // 0번에서부터 pivot으로설정하고 뒤로 검사, 작은게 있으면 변경, 위치 변경
void insertion_sort(int* A, int n);
void bubble_sort(int* A, int n);
int ascend(int x, int y);   // 오름차순 비교함수
...
//    selection_sort(list, 4);                 // 선택 정렬
//    printArray(list, 4, "Selection");        // 결과 출력
